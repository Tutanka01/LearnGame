...
### Authentification (`src/lib/session.ts` + `src/lib/auth.ts` + `src/lib/oidc.ts` + `src/app/api/auth/*`)

Le noyau est **pur et testable** (`session.ts` : scrypt versionné `scrypt$N$r$p$salt$hash` + sessions en base), `auth.ts` ne fait que coller le cookie httpOnly. Décisions structurantes :

- **Sessions en base** (`auth_sessions`) : token aléatoire stocké **uniquement en SHA-256** (une fuite de base ne permet pas de forger un cookie), **pivoté à chaque connexion**, **révoqué** à la déconnexion (DELETE en base, pas juste un cookie jeté) et **morte aussitôt** si le compte repasse `status != 'approved'`. Durée fixe 30 jours ; purge des sessions expirées à chaque création.
- **`authValidate.ts`** = source de vérité unique des règles (pseudo 3–32, mot de passe ≥ 8, force, chemin `?next=` sûr via `isSafeLocalPath`) : importé par les routes API **et** le formulaire client — les messages affichés sont exactement ceux du serveur.
- Défenses : `assertSameOrigin()` (CSRF en profondeur sur les routes d'auth), `burnScryptTime()` (coût scrypt payé même si le compte n'existe pas → anti-énumération par timing — aussi appliqué aux comptes SSO, dont `password_hash` vaut `''`), double fenêtre anti-brute-force (par IP et par couple IP+compte), **re-hachage transparent** des anciens hash `salt:hash` à la première connexion réussie (`needsRehash`).
- `getCurrentUser()` retourne un `SessionUser` **sanitisé** (jamais `password_hash`) — c'est le type attendu par tous les appelants.
- **Connexion SSO OIDC** (`src/lib/oidc.ts` + `/api/auth/oidc/start|callback`, détails dans `docs/authentification.md` §8) : la seule dépendance d'authentification du projet est `openid-client` v6, adoptée pour la validation cryptographique des jetons. Flux authorization code + PKCE S256 ; `state` single-use haché en base (`oidc_flows`, TTL 10 min) + **cookie de binding** `lg_oidc_flow` (anti login-CSRF) ; signature du ID token vérifiée (JWKS) avec allowlist locale d'algs asymétriques (`none`/`HS*` refusés — le CAS les annonce) ; `redirect_uri` de l'échange reconstruit depuis celui stocké dans le flux. Identité = `(oidc_issuer, oidc_sub)` ; première connexion → compte **approved** sans mot de passe local, rattachement au compte local par e-mail (le claim `email_verified` ABSENT ne bloque pas — le CAS l'omet), jamais de confiance dans le pseudo (suffixe si collision). Aucun token conservé. Les redirections internes passent par `redirectOrigin()` : `req.url` est réécrit en `localhost` par `next start`, ne JAMAIS s'en servir pour construire des URL.
- Le formulaire `/login` (server component qui redirige les connectés vers `/`) délègue à `LoginForm` : validation en direct (disponibilité du pseudo via `/api/auth/username-available`, jauge de force, Verr. Maj), erreurs de champ par `aria-invalid`/`aria-describedby`, écran dédié au compte en attente d'approbation, bouton SSO si configuré (prop `oidc`), erreurs du callback SSO affichées en bannière (`?error=<code fermé>`).
- Tests : `tests/authCore.test.ts` (noyau pur, 47 règles), `tests/authApproval.test.ts` (workflow d'approbation, 2 phases avec redémarrage simulé) et `tests/oidcCore.test.ts` (SSO, 25 vérifications, IdP mocké sans réseau via cache pré-chargé).
...
`OPENAI_BASE_URL` / `OPENAI_API_KEY` / `OPENAI_MODEL` (endpoint OpenAI-compatible) · `OPENAI_MAX_TOKENS` (garder large, ≥ 50000 : le thinking consomme ce budget) · `OPENAI_REASONING_EFFORT` (`none` recommandé, `default` = ne rien envoyer) · `OPENAI_TEMPERATURE` (0.6 par défaut) · `SESSION_SECURE_COOKIE` (`0` pour autoriser le cookie de session en HTTP pur, sinon `secure` en prod) · `TRUST_PROXY` (`1` derrière un proxy qui écrase `x-forwarded-*` : fenêtres anti-abus par IP réelle ET origine des redirections SSO — le compose intégré l'injecte avec cette valeur par défaut) · `ADMIN_USERNAMES` (comptes promus admin : ils valident les inscriptions en attente via /admin) · Déploiement Docker : `APP_DOMAIN` + `HTTPS_MODE` (`off`/`self`/`certs` — certificats de l'opérateur dans `./certs/`, cf. `docker-compose.yml` et `docs/guide-demarrage.md` §7) · SSO : `OIDC_ISSUER` + `OIDC_CLIENT_ID` + `OIDC_CLIENT_SECRET` (activent le bouton SSO), `OIDC_REDIRECT_URI` (override explicite ; sinon dérivé de `https://$APP_DOMAIN` en HTTPS), `OIDC_SCOPES`, `OIDC_ALLOWED_DOMAINS`, `OIDC_TOKEN_AUTH` (détails : `docs/authentification.md` §8). NB : `SESSION_SECRET` est obsolète depuis la refonte des sessions (tokens aléatoires en base, plus rien à signer).
