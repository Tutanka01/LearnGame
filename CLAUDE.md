...
### Authentification (`src/lib/session.ts` + `src/lib/auth.ts` + `src/app/api/auth/*`)
...
- **`authValidate.ts`** = source de vérité unique des règles (pseudo 3–32, mot de passe ≥ 8, force) : importé par les routes API **et** le formulaire client — les messages affichés sont exactement ceux du serveur.
- Défenses : `assertSameOrigin()` (CSRF en profondeur sur les routes d'auth), `burnScryptTime()` (coût scrypt payé même si le compte n'existe pas → anti-énumération par timing), double fenêtre anti-brute-force (par IP et par couple IP+compte), **re-hachage transparent** des anciens hash `salt:hash` à la première connexion réussie (`needsRehash`).
- `getCurrentUser()` retourne un `SessionUser` **sanitisé** (jamais `password_hash`) — c'est le type attendu par tous les appelants.
- Le formulaire `/login` (server component qui redirige les connectés vers `/`) délègue à `LoginForm` : validation en direct (disponibilité du pseudo via `/api/auth/username-available`, jauge de force, Verr. Maj), erreurs de champ par `aria-invalid`/`aria-describedby`, écran dédié au compte en attente d'approbation.
- Tests : `tests/authCore.test.ts` (noyau pur, 47 règles) et `tests/authApproval.test.ts` (workflow d'approbation, 2 phases avec redémarrage simulé).
...
`OPENAI_BASE_URL` / `OPENAI_API_KEY` / `OPENAI_MODEL` (endpoint OpenAI-compatible) · `OPENAI_MAX_TOKENS` (garder large, ≥ 50000 : le thinking consomme ce budget) · `OPENAI_REASONING_EFFORT` (`none` recommandé, `default` = ne rien envoyer) · `OPENAI_TEMPERATURE` (0.6 par défaut) · `SESSION_SECURE_COOKIE` (`0` pour autoriser le cookie de session en HTTP pur, sinon `secure` en prod) · `TRUST_PROXY` (`1` derrière un proxy qui écrase `x-forwarded-for` : fenêtres anti-abus par IP réelle) · `ADMIN_USERNAMES` (comptes promus admin : ils valident les inscriptions en attente via /admin). NB : `SESSION_SECRET` est obsolète depuis la refonte des sessions (tokens aléatoires en base, plus rien à signer).
