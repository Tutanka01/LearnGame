...
| Conteneur `learngame` « unhealthy » (le proxy ne démarre jamais)      | Version antérieure de l'image : Next écoutait sur l'hostname du conteneur, pas sur 127.0.0.1 (le healthcheck échouait). Corrigé dans le Dockerfile (`HOSTNAME=0.0.0.0`) : `git pull` puis `docker compose up -d --build`.                    |
| Sur Linux : l'app crash au premier lancement (base SQLite illisible)  | Le dossier `./data` monté dans le conteneur appartient à `root` (créé par Docker) alors que l'app tourne sous l'utilisateur `node` (uid 1000). Une seule fois : `sudo chown -R 1000:1000 data` puis `docker compose restart`.                |
